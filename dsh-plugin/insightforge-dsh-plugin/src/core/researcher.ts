/**
 * InsightForge 核心协调器 —— MarketResearcher(适配 dsh 插件版)
 *
 * 改编自 backend/src/agents/MarketResearcher.ts 的核心编排逻辑:
 *   1. 关键词提取 (LLM)
 *   2. 多源数据采集 (Aggregator)
 *   3. 报告生成 (LLM)
 *
 * 适配点:
 * 1. 改为 InsightForgeCore 类,可注入 Config 与 LLM 调用
 * 2. 引入 depth 档位(quick/standard/deep)映射 LLM 参数
 * 3. 引入缓存层(基于 cacheEnabled 配置)
 * 4. 引入并发控制(Semaphore)
 * 5. dispose() 钩子供 ctx.effect 注册
 */
...
export class InsightForgeCore {
  ...
  constructor(config: Config) {
    ...
    getDb(config);
    logger.info({ provider: config.llmProvider, dbPath: config.dbPath }, 'InsightForgeCore 已初始化');
  }

  /** 公开 dispose,供 ctx.effect 注册(NFR-04) */
  dispose(): void {
  ...
  /**
   * 全流程调研入口(对应 backend MarketResearcher.run)
   */
    ...
    return this.semaphore.run(async () => {
      ...
      try {
        ...
        if (this.reportCache && !req.noCache) {
          ...
          if (cached) {
            logger.info({ projectId, depth, cacheKey }, '命中缓存');
        ...
        logger.info({ projectId, keywords }, '关键词提取完成');
        ...
        logger.info({ projectId, inserted: aggregateResult.inserted }, '数据采集完成');

        // 步骤 3: 报告生成
        session.progress = '正在整合数据生成报告...';
        const dataContext = await this.buildContext(projectId, profile.searchLimit);
